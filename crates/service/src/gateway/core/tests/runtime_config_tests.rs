...
fn reload_from_env_updates_timeout_and_proxy() {
    ...
    let _image_strip_tool_guard = EnvGuard::set(ENV_CODEX_IMAGE_GENERATION_STRIP_TOOL, "1");
    ...
    assert!(codex_image_generation_strip_tool_enabled());
...
fn reload_from_env_defaults_keep_request_gate_legacy_unbounded() {
    ...
    let _image_strip_tool_guard = EnvGuard::clear(ENV_CODEX_IMAGE_GENERATION_STRIP_TOOL);
